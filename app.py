...
def one_user (user_id):
    if request.method == 'GET':
        ...
        if user is None:
            return 'Тайкой пользователь не найден'
        else:
        ...
        if user is None:
            return 'Тайкой пользователь не найден', 404
        ...
        db.session.commit()
        db.session.close()
        return f'Обьект с id {user_id} успешно изменен', 200
        ...
        db.session.commit()
        db.session.close()
        return f'Обьект с id {user_id} успешно удален', 200
...
def one_order (order_id):
    if request.method == 'GET':
        ...
        if order is None:
            return 'Тайкой заказ не найден', 404
...
def one_offer (offer_id):
    if request.method == 'GET':
        ...
        offer = db.session.query(Offer).get(offer_id)
        if offer is None:
            return 'Тайкое предложение не найден', 404
